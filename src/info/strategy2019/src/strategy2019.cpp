...
#define FAKE_UNMOVE_TEST_ENABLE false //发的位置是飞行到的，测试用
#define NO_ACCURATE_X_MODE false  //用声纳

using std::vector;
using ros::Publisher;


float REFRESH_RATE = 10; //发出位置点的刷新帧率
float REFRESH_PERIOD; //刷新周期

float BOARD_AIM_HEIGHT = 0.5; //对板子的高度
float GO_THROUGH_CIRCLE_HEIGHT = 1.45;  //钻圈高度
float PILLAR_AIM_HEIGHT = 1.0; //对柱子高度
float GO_THROUGH_CIRCLE_DISTANCE = 1.5;//钻圈往前飞距离
float FLYING_OVER_PILLAR_HEIGHT = 1.5;//越过柱子的高度
...
float HEIGHT_MAX = 3; //最大飞行高度
float HEIGHT_MIN = 0.35; //最小飞行高度

float TOLLERANCE_Z; //判断是否到达的容忍
float TOLLERANCE_XY;//xy的容忍
...
float LEFT_MOVING_MAX = 3.6;  //相对于中点能飞的最远距离
float RIGHT_MOVING_MAX = -3.6;//

float LEFT_NET_BOUND = 3.6; //网的边界
float RIGHT_NET_BOUND = -3.6;

//状态量
enum STATE_TYPE {
    TARGET_CIRCLE_NUM,//钻圈
    STATE_DESTPOS_ARRIVED,//到达目标点
    STAGE_NUM,//状态 阶段
    STARTPOS_IS_USEFUL,//开始飞时为1，飞到目标点为0
    TARGET_DETECTED,//目标检测到
    TARGETPOS_UPDATED,//判断检测到的目标点手否更新，超过一定帧数没有，则认为丢失，重新搜寻
    SONAR_USEFUL//声呐用的时候改为1
};
...
std::map<STATE_TYPE, int> stateold;//判断状态更新时用

Publisher pub_Pose;//发布 要去的位置，发给px34auto
Publisher pub_Vel;//发布数组
Publisher pub_TakeOff;//发布 我要起飞
Publisher pub_camCmd;//发布，sig_info
Publisher pub_FakePose;//fakemode 假装px4发回的位置
Publisher pub_FakeStatus;//px4发回的状态 such as offboard之类的

vec3f_t currentPosIdeal = {0.0, 0.0, 0.0};//虚拟坐标系的理想位置,自己真正在的位置，去除飞行累计误差
vec3f_t setpointPosIdeal;//理想坐标系下设置飞行目标点

vec3f_t currentPX4Pos = {0.0, 0.0, 0.0};//从飞机拿回来的当前位置
vec3f_t currentVel = {0.0, 0.0, 0.0};//我现在认为飞机的速度，速度规划时候的过程量
//vec3f_t destPX4Pos = {0.0, 0.0, 0.0};
vec3f_t startPX4Pos;//上一个时刻从px4拿回的位置
vec3f_t originPX4Pos = {0, 0, 0};//最开始从从飞机拿到的位置

vec3f_t targetPos;//目标相对位置，三维信息，xyz
vec3f_t BIAS_TO_YELLOW_BOARD, BIAS_TO_PILLAR;//相对于黄板的要飞的位置

vector<Target> Targets;//目标

std::vector<SearchDirection> circleDirection; //人为规划的方向

uint8_t autonomy_status;//px4读过来的状态

u_int8_t MoveMethodBetween[17][17]; //一个柱子到另一个柱子的运动方法

//bool QRStarted = false;

//int currentTargetID = 0;

std::queue<vec3f_t> poseRecord;//时间延时记录，stamp记录一段时间的位置


//region function declaration
void getParas(ros::NodeHandle &n);//从yxml读参数

void InitPlaces();//初始划圈和柱子的方向


void updatePosToIdeal();//飞机拿回的位置，减去上一时刻的位置的差，更新到理想坐标系中

void sendAutonomyPos();//根据当前的位置，目标点，速度，计算出下一时刻的飞机位置，发给px4


void calNext(vec3f_t velToThisDirection, vec3f_t disToThisDirection, vec3f_t &outputvel, vec3f_t &outputdis,
             float MAX_ACC, float MAX_VEL);//被调用，接口，计算当前时刻，当前位置，最大加速度，最大速度下的速度和位移

void sendGentlyStopPos();//缓慢停下

void MoveBy(bool is_precise, float disX, float disY, float disZ = 0.0);//是否用小的容忍度，否则用大的，位移相对xy

void MoveTo(bool is_precise, vec3f_t target);//位移绝对xy

void MoveToZ(bool is_precise, float Z);//z位移

void StartMoveBy(bool is_precise, float disX, float disY, float disZ = 0.0);//扫的时候用

void StartMoveTo(bool is_precise, vec3f_t target);

void EndMove();//结束扫描

void PillarFromTo(int startPillar, int endPillar);//从几号柱子到几号

void aimToApron();//对准停机坪

void aimToCircle();//对板子

void aimToPillar();//对柱子
//callback
...
void CB_outputTimer(const ros::TimerEvent &e);//每段时间调用，输出当前速度位置信息

void CB_camSigTimer(const ros::TimerEvent &e);//发sig_info

void CB_postionRecordTimer(const ros::TimerEvent &e);//记录延时位置
...
void resetCurrentPosTo(float x, float y, float z);//校正当前位置
...
int main(int argc, char **argv) {
    ...
    ros::Timer outputTimer = n.createTimer(ros::Duration(1.5), CB_outputTimer);
    ros::Timer camSigTimer = n.createTimer(ros::Duration(1 / 30.0), CB_camSigTimer);
    ros::Timer postionRecordTimer = n.createTimer(ros::Duration(1 / 10.0), CB_postionRecordTimer);//记录一段时候的位置
    ...
    ////Stage 0 : Aim to the apron 对准停机坪
    ...
    while (targetnum < 3) //todo
    {
        ...
        while (ros::ok())//左右扫，找
        {
            if (Targets[targetnum].toDir == TOWARD_LEFT ||
                (Targets[targetnum].toDir == UNSET && currentPosIdeal.y >= 0))  //往左还是右扫
            {
            ...
            while (state.at(STATE_DESTPOS_ARRIVED) == 0 && ros::ok())//某一次的向左右
            {
                state.at(SONAR_USEFUL) = 1;
                ROS_WARN_STREAM_DELAYED_THROTTLE(2, "............ LOOKING FOR " << targetnum);
                ros::spinOnce();
                updatePosToIdeal();
                vec3f_t error = currentPosIdeal - setpointPosIdeal; //误差
                ...
                sendAutonomyPos(); //发下一个时刻的位置

                loop_rate.sleep();
            }
            if (state.at(TARGET_DETECTED) == 1)
            {
                ROS_WARN_STREAM("................. TARGET DETECTED! ");
                EndMove();
                break;
            }
            if (state.at(STATE_DESTPOS_ARRIVED) == 1)
            {
                ROS_WARN_STREAM("................. arrive at edge! change direction.");
        ...
        state.at(SONAR_USEFUL) = 0; //扫描结束，不用sonar
        aimToCircle(); //对准
...
void CB_outputTimer(const ros::TimerEvent &e) {
    ...
    ROS_INFO_STREAM(std::fixed << std::setprecision(2) << "NOW: " << currentPosIdeal.toString() << " DEST: "
                               << setpointPosIdeal.toString() << " MOVE "
                               << goal.toString() << " PX4Pos: "
                               << currentPX4Pos.toString() << " velSP: " << currentVel.toString());
    if(currentPosIdeal.y>LEFT_MOVING_MAX || currentPosIdeal.y<RIGHT_MOVING_MAX || currentPosIdeal.z >HEIGHT_MAX)
        ROS_WARN_STREAM("NOW pos out of range! ");
...
void sendAutonomyPos() {
    ...
    {
        ROS_WARN("[MOVE] Ready to MOVE, but status is not 4 or 5!!");
        return;
    }
...
void aimToCircle() {
    ROS_WARN_STREAM("...............[AIM TO CIRCLE] " << state.at(TARGET_CIRCLE_NUM));
...
//        ROS_INFO_STREAM_DELAYED_THROTTLE(2, "dis2adjust: " << error.toString());
    ...
    ROS_WARN_STREAM("................[AIM TO CIRCLE " << state.at(TARGET_CIRCLE_NUM) << " COMLETED]");
}
...
void CB_Sonar(const geometry_msgs::PointConstPtr &msg) {
    ...
    if (state.at(SONAR_USEFUL) == 1)
    {
        if (setpointPosIdeal.y > 0)
        {
            ...
            {
                ROS_WARN_STREAM("....edge reset y to " << LEFT_NET_BOUND - leftdis);
                currentPosIdeal.y = LEFT_NET_BOUND - leftdis;
            } else{
                if (fabsf(currentPosIdeal.y - LEFT_MOVING_MAX) < 1.5)
                {
                    ROS_WARN_STREAM("....edge reset y to " << LEFT_NET_BOUND - 1.5f);
                    currentPosIdeal.y = LEFT_NET_BOUND - 1.5f;
                }
            }
        } else
        {
            ...
            {
                ROS_WARN_STREAM("....edge reset y to " << RIGHT_NET_BOUND - rightdis);
                currentPosIdeal.y = RIGHT_NET_BOUND - rightdis;
            } else{
                if (fabsf(currentPosIdeal.y - RIGHT_NET_BOUND) < 1.5)
                {
                    ROS_WARN_STREAM("....edge reset y to " << RIGHT_NET_BOUND - 1.5f);
                    currentPosIdeal.y = RIGHT_NET_BOUND - 1.5f;
                }
            }
        }
    }
...
void CB_postionRecordTimer(const ros::TimerEvent &e) {
    if (poseRecord.size() < 10)
        poseRecord.push(currentPosIdeal);
    else
    {
        poseRecord.push(currentPosIdeal);
        poseRecord.pop();
...
void resetCurrentPosTo(float x, float y, float z) {
    vec3f_t newpos{x, y, z};
    vec3f_s offset;
    offset = newpos - currentPosIdeal;
    currentPosIdeal = newpos;
    currentPosIdeal.time = ros::Time::now();
    for (int i = 0; i < poseRecord.size(); i++)
    {
        vec3f_t temp;
        temp = poseRecord.front();
        poseRecord.pop();
        temp = temp + offset;
        poseRecord.push(temp);
    }
}
