...
def detect(current_total_dect,current_image,current_depth_img=[]):
    ...
    # for i in range(len(current_total_dect.result)):
        # print(current_total_dect.result[0].label)
...
def doit():
        ...
        pub_total=rospy.Publisher("/total_info",total,queue_size = 50)
        get_total_update=0
        local_flag=0
            ...
            if color_img_update == 1 and depth_img_update==1 : #and sig_info_update==1
                if(stage==0):
                    if(local_flag==0):
                        ...
                        et, frame = capture.read()
                    # rospy.loginfo(local_flag)
                    #cv2.imshow('frame', frame)
                    if et:
                        if len(frame)!=0:
                            get_total=detect(total_dect,frame)
                            # print("cahndf="+str(len(frame)))
                            del frame
                            get_total_update=1
                            et=False
                ...
                if  get_total_update==1:
                    # rospy.loginfo("length "+str(len(get_total.result)))
                    # rospy.loginfo(get_total)
                    get_total_update=0
                        #print(frame.shape)
                color_img_update = 0
                depth_img_update =0
