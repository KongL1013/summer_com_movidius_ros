...
def build_argparser():
    ...
    args.add_argument("-m", "--model", help="Required. Path to an .xml file with a trained model.",
                      default="/home/dd/inter-movidius-test/yolo-tiny/tensorflow-yolo-v3/frozen_darknet_yolov3_model.xml", type=str)
...
def detect(current_image,current_depth_img):
    global fx,fy,cx,cy,stage
...
# def find_pos(xmin,ymin,xmax,ymax,current_depth_img):
#     depth_box_width=xmax-xmin
#     depth_box_height=ymax-ymin
#     print(depth_box_width)
#     delta_rate=0
#     x_box_min=xmin+depth_box_width*delta_rate
#     y_box_min=ymin+depth_box_height*delta_rate
#     x_box_max=xmax-depth_box_width*delta_rate
#     y_box_max=ymax-depth_box_height*delta_rate
...
#     y_pos = (0.5 * (y_box_min + y_box_max) - cy) * z_pos / fy;
    ...
    # cv2.rectangle(cv_image, (x_box_min,y_box_min), (x_box_max,y_box_max), (0,0,0), 2)
    # cv2.putText(cv_image,str(z_pos),(x_box_min, y_box_min), cv2.FONT_HERSHEY_COMPLEX, 0.6, (0,255,0), 1)
...
def doit():
        ...
        while not rospy.is_shutdown():
            if color_img_update == 1 and depth_img_update==1 and sig_info_update:
                if(stage==0):
                    detect(cv_image,depth_img)
                if(stage==1):
                    pass
                if(stage==2):
...
def my_listener():
    ...
    dododo=process()
    sig_info=rospy.Subscriber("/sig_info",Int16,dododo.sig_info_callback)
